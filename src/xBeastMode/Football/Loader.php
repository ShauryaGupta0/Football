                ...
                if($sender instanceof Player){
                    if(count($args) > 0){
                        switch(strtolower($args[0])){
                            case "small":
                                $nbt = Entity::createBaseNBT($sender->getPosition());
                                $ent = Entity::createEntity("BallEntity", $sender->getWorld(), $nbt);
                                $sender->sendMessage(TextFormat::GREEN . "Added new small ball.");
                                $ent->spawnToAll();
                                $sender->getWorld()->addSound($sender->getPosition(), LevelSoundEventPacket::SOUND_BLOCK_END_PORTAL_FRAME_FILL);
                                break;
                            case "medium":
                                $nbt = Entity::createBaseNBT($sender->getPosition());
                                $ent = Entity::createEntity("BallEntity", $sender->getWorld(), $nbt);
